...
/* USER CODE BEGIN PV */
volatile uint32_t IdleLoopCount = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_NVIC_Init();
  /* USER CODE BEGIN 2 */
  BearDriver_Main();  /* never returns — contains its own infinite loop */
  /* USER CODE END 2 */
...
static void MX_NVIC_Init(void)
{
  ...
  /* TIM20_BRK_IRQn interrupt configuration */
  HAL_NVIC_SetPriority(TIM20_BRK_IRQn, 4, 0);
